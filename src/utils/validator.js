...
const validator = {
  ...
  numberValue: (rule, value, callback) => {
    if (isNumber(rule.minValue) && value < rule.minValue) {
      callback(new Error(t('validation.error.numberMinValue', [rule.minValue])));
      return;
    }
    if (isNumber(rule.maxValue) && value > rule.maxValue) {
      callback(new Error(t('validation.error.numberMaxValue', [rule.maxValue])));
      return;
    }
    callback();
  },
  ...
  alintunderline: (rule, value, callback) => {
    ...
    if (/^([a-zA-Z0-9_]+)$/.test(value)) {
      ...
      callback(new Error(t('validation.error.alintunderline')));
    }
  },
  ...
  date: (rule, value, callback) => {
    ...
    const dt = new Date(Number.parseInt(result[1], 10), Number.parseInt(result[2], 10) - 1, Number.parseInt(result[3], 10));
    if (dt.getFullYear() === Number.parseInt(result[1], 10) && dt.getMonth() === Number.parseInt(result[2], 10) - 1 && dt.getDate() === Number.parseInt(result[3], 10)) {
      callback();
    } else {
      callback(new Error(t('validation.error.date')));
    }
  ...
  datetime: (rule, value, callback) => {
    ...
    if (result != null && result.length > 0) {
      const dt = new Date(Number.parseInt(result[1], 10), Number.parseInt(result[2], 10) - 1, Number.parseInt(result[3], 10), Number.parseInt(result[4], 10), Number.parseInt(result[5], 10), Number.parseInt(result[6], 10));
      if ((dt.getFullYear() === Number.parseInt(result[1], 10) && dt.getMonth() === Number.parseInt(result[2], 10) - 1 && dt.getDate() === Number.parseInt(result[3], 10) && dt.getHours() === Number.parseInt(result[4], 10) && dt.getMinutes() === Number.parseInt(result[5], 10) && dt.getSeconds() === Number.parseInt(result[6], 10))) {
        callback();
      } else {
